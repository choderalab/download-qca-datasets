...
outputfile = h5py.File('RNA-DIVERSE-OPENFF-DEFAULT.hdf5', 'w')
...
for subset in config['subsets']:
    ...
    for name in recs_by_name:
        ...
        #ds = group.create_dataset('formation_energy', data=np.array([vars_b3lyp['DFT TOTAL ENERGY']+float(vars_d3bj['DISPERSION CORRECTION ENERGY'])-ref_energy for vars_b3lyp,vars_d3bj in zip(qcvars_b3lyp,qcvars_d3bj)]), dtype=np.float64)
        ...
        for value in config['values']:
            ...
            try:
                ...
                ds = group.create_dataset(key, data=np.array([vars[value] for vars in qcvars], dtype=dtype), dtype=dtype)
                if key in units:
