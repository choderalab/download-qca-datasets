...
"""
Note: 4060/7560 record entries were completed. Extract completed record entries from logging information. 
"""
...
for subset in config['subsets']:
    ...
    x = ds.status(["default-dlc"])
    N_MAX = int(x['default-dlc']['COMPLETE'])
    if INDEX_END > N_MAX:
        INDEX_END = N_MAX
    ...
    for i, record_name in enumerate(record_names):
        opt_record = ds.get_record(record_name, specification="default-dlc")
